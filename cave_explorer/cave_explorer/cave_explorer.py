    ...
    def __init__(self):
        ...
        try:
            ...
            if not os.path.exists(model_path):
                self.get_logger().error(f"L Model file not found at: {model_path}")
                self.use_custom_model = False
            elif not os.path.exists(repo_path):
                self.get_logger().error(f"L YOLOv5 repo not found at: {repo_path}")
                self.use_custom_model = False
            else:
            ...
            self.get_logger().error(f"L Could not import PyTorch: {e}")
            self.get_logger().error("PyTorch is not installed in the current Python environment")
            self.use_custom_model = False
            
        except Exception as e:
            self.get_logger().error(f"L Could not load custom model: {e}")
